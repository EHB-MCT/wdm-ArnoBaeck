...
app.post("/event", (request, response) => {
  ...
  const events = db.get(event.session_id) || [];
  events.push(event);
  db.set(event.session_id, events);

  response.status(204).end();
});

// Convert raw events into statistical features
function buildFeatures(events) {
  const clicks = (target) => events.filter((event) => event.type === "click" && event.target === target).length;
  const hovers = (target) => events.filter((event) => event.type === "hover" && event.target === target).map((event) => event.hover_ms || 0);
...
app.delete("/reset", (_request, response) => {
  db.clear();
