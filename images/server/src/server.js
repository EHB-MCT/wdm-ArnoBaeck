...
app.post("/session-event", authenticateToken, async (request, response) => {
	...
	try {
		sessionEvent.user_id = new ObjectId(request.userId);
...
app.get("/profile", authenticateToken, async (request, response) => {
	try {
		...
		const sessions = await sessionsCollection.find({ user_id: new ObjectId(request.userId) }).toArray();
...
app.delete("/reset", authenticateToken, async (request, response) => {
	try {
		...
		await sessionsCollection.deleteMany({ user_id: new ObjectId(request.userId) });
